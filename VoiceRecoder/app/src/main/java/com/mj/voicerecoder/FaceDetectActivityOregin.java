...
public class FaceDetectActivityOregin extends BaseActivity {
        ...
        private void verify(JSONObject obj) throws JSONException {
            ...
            if ("success".equals(obj.get("rst"))) {
                if (obj.getBoolean("verf")) {
                    showTip("通过验证，欢迎回来！");
                    Voice.getInstance(FaceDetectActivityOregin.this).startSpeed(mAuthid+"出现");
                } else {
                    showTip("验证失败，不是同一个人");
                }
            } else {
...
//                startActivityForResult(mIntent, REQUEST_CAMERA_IMAGE);
    ...
    private class FaceTask extends AsyncTask<Void, Void, Void> {
        ...
        private void savePic(Bitmap bmp) {
            String fileSrc = null;
            if(null != bmp){
                FaceUtil.saveBitmapToFile(FaceDetectActivityOregin.this, bmp);
            }

            // 获取图片保存路径
            fileSrc = FaceUtil.getImagePath(FaceDetectActivityOregin.this);
            // 获取图片的宽和高
